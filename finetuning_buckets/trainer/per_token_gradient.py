...
class ConstrainedSFTTrainer(Trainer):
    ...
    def __init__(
        ...
        bias_length: int = 5,
        first_token_bias_factor: float = 2,
    ...
    def compute_reference_log_probs(self, padded_batch: Dict) -> Dict:
        ...
        # consider the gradient magnitude of only the first few tokens.
        ...
        if self.model_family == "gemma":
            ...
            raise ValueError(f"Model family {self.model_family} is not supported.")
        
        
        gradient_magnitude = torch.zeros_like(reference_logps_full).unsqueeze(-1).repeat(1, 1, num_blocks + 1)
        num = len(reference_logps_full)
        for i in range(num):
            item = reference_logps_full[i]
            cnt = 0
            for j, token_logp in enumerate(item):
                if token_logp <= 0: # valid token
                    loss = -token_logp
                    loss.backward(retain_graph=True)
                    #gradient_magnitude[i][j] = torch.nn.utils.clip_grad_norm_(self.ref_model.parameters(), max_norm=float('inf')).detach()
                    for k in range(num_blocks):
                        if k == num_blocks - 1:
                            gradient_magnitude[i][j][k] = torch.nn.utils.clip_grad_norm_(self.ref_model.lm_head.parameters(), max_norm=float('inf')).detach()
                        else:
                            gradient_magnitude[i][j][k] = torch.nn.utils.clip_grad_norm_(self.ref_model.model.layers[k].parameters(), max_norm=float('inf')).detach()
                    
                    gradient_magnitude[i][j][num_blocks] = torch.nn.utils.clip_grad_norm_( list(self.ref_model.model.layers[-3:].parameters()) + list(self.ref_model.lm_head.parameters()),
                         max_norm=float('inf')).detach()

                    print(f'grad_norm-{cnt}:', gradient_magnitude[i][j][-1])
                    cnt += 1
        ...
        reference_logps, reference_logps_avg, reference_logps_full, gradient_magnitude = reference_logps.detach(), reference_logps_avg.detach(), reference_logps_full.detach(), gradient_magnitude.detach()
        return reference_logps, reference_logps_avg, reference_logps_full, gradient_magnitude
    ...
    def get_train_dataloader(self) -> DataLoader:
        ...
        if (self.ref_model is not None) and (not self._precomputed_train_ref_log_probs):
            ...
            for padded_batch in tqdm(iterable=data_loader, desc="Train dataset reference log probs"):
                ...
                reference_logp, reference_logp_avg, reference_logp_full, gradient_magnitude = self.compute_reference_log_probs(padded_batch)
                reference_logp, reference_logp_avg, reference_logp_full, gradient_magnitude = self.accelerator.gather_for_metrics(
                    (reference_logp, reference_logp_avg, reference_logp_full, gradient_magnitude)
                )
                ...
                gradient_magnitude_list.append(gradient_magnitude.cpu())
            ...
            gradient_magnitude_final = []
            for items in gradient_magnitude_list:
                len_items = len(items)
                for item_id in range(len_items):
                    item = items[item_id].float().numpy()
                    gradient_magnitude_final.append(item)
            ...
            self.train_dataset = self.train_dataset.add_column(
                name="gradient_magnitude", column=gradient_magnitude_final
            )
            self.label_names = ["reference_logps", "reference_logps_avg", "reference_logps_full", "gradient_magnitude"] # add `reference_logps` to label_names
    ...
    def save_gradient_magnitude(self, save_path):
        ...
        if (self.ref_model is not None) and (not self._precomputed_train_ref_log_probs):
            ...
            if PartialState().is_local_main_process:
                torch.save(gradient_magnitude_final, save_path)
    ...
    def get_beta_list(self, length):
        ...
        additive_bias = torch.FloatTensor( [self.bias_shift] * len_prefix )
    ...
    def soft_sft_loss(
        ...
        for i in range(num):
            ...
            beta, additive_bias = beta.to(policy_logps.device), additive_bias.to(policy_logps.device)

            losses_list = 2 * ( 1 - F.sigmoid( beta * (policy_item - reference_item) + additive_bias ) ).detach()
            losses_list = torch.clamp(losses_list, min=1e-3)

            if i == 1:
                print('weight:', losses_list[:self.bias_length].mean(), losses_list[self.bias_length:].mean())
                print('policy_ce:', -policy_item[:self.bias_length].mean(), -policy_item[self.bias_length:].mean())
